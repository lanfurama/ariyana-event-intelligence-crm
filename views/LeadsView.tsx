...
export const LeadsView: React.FC<LeadsViewProps> = ({
  ...
  const [emailLogs, setEmailLogs] = useState<
    Array<{ leadId: string; count: number; lastSent?: Date }>
  >([]);
  ...
  const [sendingProgress, setSendingProgress] = useState<
    Record<string, 'pending' | 'sending' | 'sent' | 'failed'>
  >({});
  ...
  const getEmailStatus = (leadId: string) => {
    ...
    return log
      ? { hasEmail: true, count: log.count, lastSent: log.lastSent }
      : { hasEmail: false, count: 0 };
  };
  ...
  const preparedEmails = useMemo(() => {
    ...
    return filteredLeads
      .filter((lead) => {
        ...
        if (
          selectedTemplate.leadType == null ||
          String(selectedTemplate.leadType || '').trim() === ''
        ) {
  ...
  const handleSendEmails = async () => {
    ...
    preparedEmails.forEach((p) => {
      initialProgress[p.lead.id] = 'pending';
    });
    ...
    try {
      for (const prepared of preparedEmails) {
        ...
        try {
          ...
          failedCount += 1;
          alert(
            `Failed to send to ${prepared.lead.companyName}: ${err?.message ?? 'Unknown error'}`,
          );
        }
      }
    ...
    try {
      const exportData = filteredLeads.map((lead) => {
        ...
        return {
          ...
          'Last Email Sent': emailStatus.lastSent
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-1 mt-0.5">
                          {lead.keyPersonEmail && (
                            <div className="w-1 h-1 rounded-full bg-green-500" title="Email"></div>
                          )}
                          {lead.keyPersonPhone && (
                            <div className="w-1 h-1 rounded-full bg-blue-500" title="Phone"></div>
                          )}
                          {lead.keyPersonLinkedIn && (
                            <div
              ...
              {loadingTemplates ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-500 text-sm mt-1">
                    Please create an email template first
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                        ...
                        return (
                          <div className="space-y-3">
                            ...
                            <div>
                              ...
                              <div
                                className="text-sm text-slate-900 mt-1 bg-white p-3 rounded border border-slate-200 max-h-40 overflow-y-auto"
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-slate-500">
                          {filteredLeads.length - preparedEmails.length} leads in list do not match
                          this template
                        </span>
                      </div>
                              ...
                              <div className="flex items-start justify-between mb-2">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {status && (
                                      <span className="inline-flex items-center gap-1 text-xs font-medium">
                                        ...
                                        {status === 'failed' && (
                                          <>
                                            <XCircle size={14} className="text-red-600" /> Failed
                                          </>
                                        )}
                                      </span>
                                    )}
                                  ...
                                  <p className="text-xs text-slate-500 mt-1 ml-8">
                                    To: {prepared.lead.keyPersonEmail} (
                                    {prepared.lead.keyPersonName})
                                  </p>
                                </div>
                              </div>
                  ...
                  {selectedTemplateId &&
                    preparedEmails.length === 0 &&
                    filteredLeads.some((l) => l.keyPersonEmail) && (
                      <div className="text-center py-8 text-slate-500">
                        <Mail className="text-slate-300 mx-auto mb-2" size={32} />
                        <p>
                          No leads match this template or all matching leads have already been
                          emailed.
                        </p>
                        {(() => {
                          const t = emailTemplates.find((x) => x.id === selectedTemplateId);
                          const baseTypeLabel = t?.leadType
                            ? `${t.leadType} leads`
                            : 'Regular leads';
                          ...
                          return (
                            <p className="text-xs mt-1">
                              This template currently targets {effectiveLabel}.
                            </p>
                          );
                        })()}
                      </div>
                ...
                >
                  {sendingEmails ? (
                    <>
                      ...
                      Sending (
                      {
                        Object.values(sendingProgress).filter((s) => s === 'sent' || s === 'failed')
                          .length
                      }
                      /{preparedEmails.length})
                    </>
                  ) : (
                    <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewLeadData({ ...newLeadData, companyName: e.target.value })
                      }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewLeadData({ ...newLeadData, status: e.target.value as Lead['status'] })
                      }
                      className="w-full p-2 text-sm bg-white text-slate-900 border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none"
                    >
                      <option value="New">New</option>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewLeadData({ ...newLeadData, keyPersonName: e.target.value })
                        }
                        className="w-full p-2 text-sm bg-white text-slate-900 border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewLeadData({ ...newLeadData, keyPersonTitle: e.target.value })
                        }
                        className="w-full p-2 text-sm bg-white text-slate-900 border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewLeadData({ ...newLeadData, keyPersonEmail: e.target.value })
                        }
                        className="w-full p-2 text-sm bg-white text-slate-900 border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewLeadData({ ...newLeadData, keyPersonPhone: e.target.value })
                        }
                        className="w-full p-2 text-sm bg-white text-slate-900 border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none"
                        placeholder="+84..."
                      />
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewLeadData({ ...newLeadData, keyPersonLinkedIn: e.target.value })
                        }
                        className="w-full p-2 text-sm bg-white text-slate-900 border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none"
