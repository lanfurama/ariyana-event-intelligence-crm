...
export const Dashboard: React.FC<DashboardProps> = ({ leads, loading }) => {
  ...
  const normalizeType = (t: string | undefined) =>
    t != null && String(t).trim() !== '' ? String(t).trim() : 'Regular';
  ...
  const leadByTypeStats = useMemo(() => {
    ...
    return types.map((type) => {
      ...
      const sentInPeriod =
        leadIds.size > 0 ? [...leadIds].filter((id) => filteredLeadIds.has(id)).length : 0;
      let lastSentAt: Date | undefined;
      leadIds.forEach((leadId) => {
        const data = sentLogsByLead.get(leadId);
        if (data?.lastSent && (!lastSentAt || data.lastSent > lastSentAt))
          lastSentAt = data.lastSent;
      });
      const lastSentLabel = lastSentAt
        ? lastSentAt.toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
          })
  ...
  const templateStats = useMemo(() => {
    ...
    filteredLogs.forEach((log) => {
      ...
      const entry = perTemplate.get(subject) || {
  ...
  const templateInsights = useMemo(() => {
    ...
    const repliesForTemplate = allEmailReplies.filter((reply) => leadIds.has(reply.lead_id));
    ...
    const isEnTemplate = emailTemplates.some(
      (t) =>
        (t.subject || '').trim() === selectedTemplate && (t.language || '').toLowerCase() === 'en',
    );
    if (isEnTemplate) {
    ...
    const replyRate =
      leadIds.size > 0 ? ((uniqueRepliedLeads / leadIds.size) * 100).toFixed(1) : '0.0';
    ...
    const dayOfWeekDistribution = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => ({
      name: day,
              ...
              <div className="space-y-3">
                {templateChartData.map((item, idx) => {
                  const fullTemplate = templateStats.find((t) => {
                    const shortName =
                      t.template.length > 40 ? t.template.substring(0, 40) + '...' : t.template;
                    ...
                    <table className="w-full text-sm">
                      <tbody className="space-y-2">
                        <tr className="border-b border-slate-100">
                          <td className="py-2.5 pr-4 font-medium text-slate-700 align-top w-1/3">
                            Email Subject:
                          </td>
                          <td className="py-2.5 text-slate-900 break-words">
                            {templateInsights.template}
                          </td>
                        </tr>
                        <tr className="border-b border-slate-100">
                          <td className="py-2.5 pr-4 font-medium text-slate-700 align-top">
                            Total Sent:
                          </td>
                          <td className="py-2.5 text-slate-900 font-semibold tabular-nums">
                            {templateInsights.totalSent} emails
                          </td>
                        </tr>
                        ...
                        {templateInsights.replies.length > 0 && (
                          <tr className="border-b border-slate-100">
                            ...
                            <td className="py-2.5 text-slate-900 tabular-nums">
                              {templateInsights.replies.length} /{' '}
                              {templateInsights.uniqueRepliedLeads} Unique Replies
                            </td>
                          </tr>
                        )}
