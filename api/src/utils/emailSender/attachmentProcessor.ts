...
export class AttachmentProcessor {
    ...
    return attachments
      ...
      .map((att) => {
        ...
        try {
          ...
          console.log(
            `[AttachmentProcessor] Attachment "${att.name}": ${(buffer.length / 1024 / 1024).toFixed(2)} MB`,
          );
          return {
            filename: att.name,
            content: buffer,
          ...
          console.warn(
            `[AttachmentProcessor] Buffer conversion failed for ${att.name}, using base64 string:`,
            bufferError.message,
          );
          return {
            filename: att.name,
  ...
  static extractFileAttachments(
    ...
    for (const att of attachments) {
      ...
      const base64Data = att.file_data?.includes(',') ? att.file_data.split(',')[1] : att.file_data;
