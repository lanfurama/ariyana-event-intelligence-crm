    ...
    try {
      ...
      for (const lead of newLeads) {
        try {
          ...
          if (existingLead) {
            const existingKeyPersonEmail =
              existingLead.key_person_email || existingLead.keyPersonEmail || '';
          ...
          if (companyNameLower) {
            existingLeadsMap.set(companyNameLower, {
              id: mappedLead.id,
      ...
      console.log(
        `Summary: Saved ${successCount}, Updated ${updatedCount}, Skipped ${duplicateCount}, Failed ${failCount}`,
      );
