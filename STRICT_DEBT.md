...
| `api/src/services/scheduledReportsJob.ts:~28` (`scheduled: true`)      | ~~marker removed 2026-07-16~~ — `api/` actually resolves node-cron **v3** from `api/node_modules` (api/package.json pins `^3.0.3`), where `scheduled` is valid, so tsc reported the directive unused. Root declares v4. | Sub-project #6 — unify node-cron on v4 (root + api), then migrate to `cron.schedule` + `.start()`. |
